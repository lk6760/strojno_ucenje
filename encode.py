...
if __name__ == "__main__":
    ...
    for MODEL_NAME in [
        ...
        # Custom Spotify Dataset
        p = Path('./data/Spotify_GTZAN_genres')
        filenames_spotify_gtzan = p.glob('**/*.wav')

        dataset_files = [filenames_gtzan, filenames_us8k, filenames_nsynth_train, filenames_nsynth_test, filenames_spotify_gtzan]
        dataset_names = ['gtzan', 'us8k', 'nsynth/train', 'nsynth/test', 'spotify']
        ...
        for filenames, ds_name in zip(dataset_files, dataset_names):
            ...
            existing_embeddings = os.listdir(folder)
#            if ds_name == 'spotify':
#                print(existing_embeddings)
            for f in tqdm(filenames):
#                if ds_name == 'spotify':
#                    print(str(f.stem)+'_i')
                if str(f.stem)+'.npy' in existing_embeddings:
                    continue
